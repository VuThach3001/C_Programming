...
## Topic: Mastring String, Character, I/O, Conversion, and Dynamic Memory Functions in C
...
- **`int strncmp (s1, s2, n)`**: performs the same function as `strcmp`, except that at most `n` characters from the strings are compared
...
- To use these character functions, you must include `<ctype.h>`
...
- **`int fgetc (filePtr)`**: returns the **next character** from the file identified by `filePtr`, or the value `EOF` if an **end-of-file** condition occurs. (Remember that this function returns an `int`)
...
- **`int fsetpos (filePtr, fpos)`**: sets the current file position for the file associated with `filePtr` to the value pointed to by `fpos`, which is of type `fpos_t` (defined in `<stdio.h>`). Returns `zero` on success, and `nonzero` on failure

- **`long ftell (filePtr)`**: returns the relative offset in bytes of the current position in the file identified by `filePtr`, or `−1L` onerror

- **`int printf (format, arg1, arg2, ..., argn)`**: writes the specified arguments to `stdout`, according to the format specified by the character string. Returns the number of characters written

- **`int remove (fileName)`**: removes the specified file. A non`zero value is returned on failure
...
- **`int atoi (s)`**: converts the string pointed to by s into an int, returning the result
